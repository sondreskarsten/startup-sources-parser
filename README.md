...
| Parser | Status | Reads | Writes | Distinct orgnrs (Apr 2026) |
|---|---|---|---:|---:|
| `skattefunn` | live | `skattefunn/raw/label=*/{date}.xlsx` (2 files) | `skattefunn/state/{date}.parquet` | ~25K (incl. avslag) |
| `cordis` | live | `cordis/raw/programme={fp7,h2020,horizon}/{date}.zip` | `cordis/state/programme={p}/{date}.parquet` + `cordis/state/all/{date}.parquet` | ~6.5K NO participants |
| `innovasjon_norge` | live | `innovasjon_norge/raw/{date}.csv` | `innovasjon_norge/state/{date}.parquet` | ~30K |

The Prosjektbanken parser (FORISS, EU, godkjente SkatteFUNN) is deferred — see `prosjektbanken-collector` v2 resolution which currently lives at `gs://sondre_brreg_data/raw/skattefunn_prosjektbanken_2026/`. For SkatteFUNN-specific work the `skattefunn` parser supersedes it.
